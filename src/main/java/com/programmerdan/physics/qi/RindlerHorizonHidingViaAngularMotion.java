 ...
 *
 */
public class RindlerHorizonHidingViaAngularMotion {
	...
	public static void main(String[] args) {
		(new RindlerHorizonHidingViaAngularMotion()).run();
	}

	private static final long PRECISION = 1024l;
	...
	private static final Apfloat SPEEDOFLIGHT = new Apfloat("299792.458", PRECISION); //km/s
	private static final Apfloat SPEEDOFLIGHTSQ = SPEEDOFLIGHT.multiply(SPEEDOFLIGHT);
	...
	//private Apfloat sunMass =            new Apfloat("1988500000000000000000000000000", PRECISION); // kg
	...
	//private Apfloat moonMass =           new Apfloat("73460000000000000000000", PRECISION); // kg
	...
	//private Apfloat earthMass =          new Apfloat("5972400000000000000000000", PRECISION); // kg
	...
	public void run() {
		...
		moonLocation = earthLocation.offset(moonApogee, Apfloat.ZERO, Apfloat.ZERO);
		sunLocation = earthLocation.offset(earthAphelion, Apfloat.ZERO, Apfloat.ZERO);
		
		// we're set up, let's do analytics.
		long stepsOnEdge = objectRadius.divide(objectResolution).longValue();
		long totalSteps = stepsOnEdge*2*stepsOnEdge*2*stepsOnEdge*2;
		long currentTakenSteps = 0l;
		long currentDoneSteps = 0l;
		...
		Apfloat objectRadiusSquared = objectRadius.multiply(objectRadius);
		...
		Apfloat tmpRindlerHorizonDistance = SPEEDOFLIGHTSQ.divide(objectRadius.multiply(precomputeRotationFactor));
		...
		for (double stepX = -stepsOnEdge; stepX < stepsOnEdge; stepX++) {
			...
			for (double stepY = -stepsOnEdge; stepY < stepsOnEdge; stepY++) {
				...
				for (double stepZ = -stepsOnEdge; stepZ < stepsOnEdge; stepZ++) {
					...
					Location midPoint = new Location(pointX, pointY, pointZ);
					...
					currentDoneSteps++;
					if (currentDoneSteps%100 == 0) {
						double completePercent = ((double) currentDoneSteps / (double) totalSteps) * 100d;
						System.out.println("Completed " + currentDoneSteps + " with " + currentTakenSteps + " taken, or " + String.format("%6.2f", completePercent) + "% complete.");
						if (currentDoneSteps%500 == 0 && currentTakenSteps > 0) {
							Apfloat tmpTakenSteps = new Apfloat(currentTakenSteps, PRECISION);
		...
		for (int i = 0; i < pathsUsed.length; i++) {
			System.out.print(String.format("%5d ", pathsUsed[i]));
		}
		System.out.println();
		
		aggregateGravityAcceleration = new Location(Apfloat.ZERO, Apfloat.ZERO, Apfloat.ZERO);
		gravAcceleration = new Location[] { new Location(Apfloat.ZERO, Apfloat.ZERO, Apfloat.ZERO), new Location(Apfloat.ZERO, Apfloat.ZERO, Apfloat.ZERO), new Location(Apfloat.ZERO, Apfloat.ZERO, Apfloat.ZERO) };
		...
		System.out.println("Final: Standard grav impact: \n" + aggregateGravityAcceleration.toString());
		for (int i = 0; i < sphereName.length; i++) {
			System.out.println("Final: Standard " + sphereName[i] + " grav impact: \n" + gravAcceleration[i].toString());
		}
